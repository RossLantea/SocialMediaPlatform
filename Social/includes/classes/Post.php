...
class Post {
	...
	public function submitPost($body, $user_to) {
		...
		$body = str_replace('\r\n', "\n", $body);
		$body = nl2br($body);
		$check_empty = preg_replace('/\s+/', '', $body); //Poistaa välilyönnit 
		...
		if($check_empty != "") {
			...
			$added_by = $this->user_obj->getUsername();
             //Jos käyttäjä on omalla profiililla, user_to is 'none'
			if($user_to == $added_by)
				$user_to = "none";
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
                 ...
                 //Valmistele user_to string, jotta se voidaan sisällyttää, vaikka sitä ei ole lähetetty käyttäjälle 
				if($row['user_to'] == "none") {
					$user_to = "";
				}
				else {
                ...
                //Tarkista onko henkilöllä joka postasi käyttäjä suljettuna
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					//kun 10 postausta ladattu, break
					if($count > $limit) {
						break;
					}
					else {
					...
					$end_date = new DateTime($date_time_now); //Tämänhetkinen aika
					$interval = $start_date->diff($end_date); //Ero päivien välillä 
					if($interval->y >= 1) {
						if($interval->y == 1)
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					//Aikakehys
					$date_time_now = date("Y-m-d H:i:s");
					$start_date = new DateTime($date_time); //Postauksen päivämäärä
					$end_date = new DateTime($date_time_now); //Tämänhetkinen päivämäärä
					$interval = $start_date->diff($end_date); //Ero päivien välillä 
					if($interval->y >= 1) {
						if($interval->y == 1)
							$time_message = $interval->y . " year ago"; //1 vuosi sitten
	...
	public function getSinglePost($post_id) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$row = mysqli_fetch_array($data_query); 
				...
				//Tarkista onko henkilöllä joka postasi käyttäjä tili suljettuna
				$added_by_obj = new User($this->con, $added_by);
				if($added_by_obj->isClosed()) {
					return;
				}
